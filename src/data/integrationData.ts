...
export const integrations: Integration[] = [
  {
    ...
    sections: [
      {
        title: "Overview",
        content: "This guide walks you through creating a Twilio Function that triggers on the `initiated` call status and posts to the TransUnion Call Authentication endpoint. It includes code, wiring steps for the `/Calls` API, and test/debug tips."
      },
      {
      ...
      },
      {
        ...
        content: "Paste the following code into your Twilio Function editor. This function intercepts the call initiation event and sends the caller information to the Call Authentication endpoint for authentication.",
...
client.calls.create(
  ...
  },
  {
    ...
    description: "Integrate TransUnion Call Authentication with Genesys Cloud CX using Web Services Data Actions for real-time call authentication of outbound calls.",
    products: ["scp", "bcd"],
    sections: [
      {
        title: "Overview",
        content: "This document provides step-by-step instructions for integrating Genesys Cloud CX with TransUnion's Call Authentication. This integration enables real-time caller ID authentication for outbound calls, helping to improve call answer rates and establish caller legitimacy."
      },
      {
  ...
  },
  {
    ...
    platform: "Plivo",
    description: "Integrate TransUnion Call Authentication with Plivo using PHLO visual workflows to validate destination numbers before initiating outbound calls.",
    products: ["scp", "bcd"],
    sections: [
      {
        title: "Overview",
        content: "This guide explains how to integrate Plivo PHLO with the TransUnion Call Authentication API to perform call authentication before initiating a call. The flow validates the destination number using the Call Authentication endpoint, then initiates the call and plays a notification message upon connection."
      },
      {
        title: "Use Case",
        content: "- Validate the destination number using TransUnion Call Authentication before placing a call\n- Automate the flow: **Start → Call Authentication Authentication → Initiate Call → Play Notification**\n- Ensure outbound calls are authenticated and branded to improve answer rates"
      },
      {
        title: "Prerequisites",
        content: "Before you begin, ensure you have:\n\n- A **Plivo account** with PHLO builder access\n- **TransUnion Call Authentication API credentials** (API key and endpoint)\n- **Authorized From and To numbers** in Plivo\n- Completed the TCS account setup (Account, TCS, Feature, Caller Profile, and TN Asset)"
      },
      {
  ...
  "to": "tel:+1{{Start.http.params.to}}"
      ...
      },
      {
        ...
        content: "Connect the nodes in the following order:\n\n**Start** → **Call Authentication HTTP Request (Success)** → **Initiate Call** → **Play Notification**\n\nHandle the Call Authentication **Failed** path with fallback logic if needed (e.g., skip authentication and proceed with the call, or log an error)."
      },
      {
        title: "Step 6 — Test & Validate",
        content: "1. Use **Fetch Response** in the HTTP node to confirm the Call Authentication endpoint returns the expected JSON\n2. Trigger the PHLO via API Request with `from` and `to` parameters\n3. Verify:\n   - The HTTP request returns a `200` status\n   - The call initiates successfully\n   - The notification message plays correctly"
      },
      {
        title: "Troubleshooting",
  ...
  },
  {
    ...
    description: "Integrate TransUnion Call Authentication with Amazon Connect using AWS Lambda and Secrets Manager for secure, automated call authentication of outbound calls.",
    products: ["scp", "bcd"],
    sections: [
      {
        title: "Overview",
        ...
        code: `// CloudWatch Log — REST API Call
{
  ...
  },
  {
    ...
    description: "Integrate TransUnion Call Authentication with NICE inContact (CXone) using Studio scripts to authenticate outbound calls via the Call Authentication REST API.",
    products: ["scp", "bcd"],
    sections: [
      {
        title: "Overview",
        content: "This guide describes how to integrate NICE inContact (CXone) with TransUnion's Call Authentication service using a Studio script. The integration uses a SNIPPET action to make a REST API call to the Call Authentication endpoint before each outbound call, authenticating the caller ID and reducing spam flagging."
      },
      {
        title: "Architecture",
      ...
      },
      {
        ...
        content: "This is the core of the integration. The SNIPPET action formats phone numbers, creates a REST API proxy, and calls the Call Authentication endpoint.\n\n1. Double-click the **SNIPPET** action\n2. Set the **Caption**: `Integration`\n3. Paste the following code into the snippet editor:",
...
ELSE
{
  ...
  },
  {
    ...
    sections: [
      {
        title: "Overview",
        content: "This guide provides complete instructions for integrating RingCentral with TransUnion's Call Authentication using the SDK provided by the TransUnion Call Authentication support team. This integration automatically deposits STIR/SHAKEN attestation tokens for your outbound calls, ensuring they are properly authenticated and branded — reducing the risk of being marked as spam."
      },
      {
        title: "What This Integration Does",
        content: "When you make an outbound call from RingCentral, the SDK automatically:\n\n1. **Receives real-time notification** via WebSocket (50–100ms latency)\n2. **Requests STIR/SHAKEN attestation token** from TransUnion's Call Authentication API\n3. **Deposits the token** before the carrier receives your call\n4. **Verifies delivery** (optional monitoring)\n\n**Result:** Your calls display as \"Verified\" on recipient phones and show Branded Caller Data (name, logo, call reason) if configured — improving answer rates and reducing spam flagging."
      },
      {
...
{"sessionId":"s-abc123...","totalLatency":"127ms"}`,
  ...
  },
  {
    ...
    platform: "Sinch",
    description: "Enable TransUnion Call Authentication on your Sinch Elastic SIP Trunk numbers — no code required. Sinch configures the integration upon request once requirements are met.",
    products: ["scp", "bcd"],
    sections: [
      {
        title: "Overview",
